...
export default function AdminPartners() {
  ...
  const [form, setForm] = useState({ name: '', logo_url: '', sort_order: 0, is_active: true })
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ name: '', logo_url: '', sort_order: 0, is_active: true })
    setShowModal(true)
  }

  const openEdit = (item: any) => {
    setEditItem(item)
    setForm({ name: item.name || '', logo_url: item.logo_url || '', sort_order: item.sort_order || 0, is_active: item.is_active !== false })
    setShowModal(true)
  }

  const handleSave = async () => {
    if (!form.name) { toast.error('Name is required'); return }
    const data = { name: form.name, logo_url: form.logo_url || null, sort_order: parseInt(form.sort_order) || 0, is_active: form.is_active }
    if (editItem) {
      const { error } = await supabase.from('partners').update(data).eq('id', editItem.id)
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-3 text-center py-10 text-gray-400">No partners yet</div>
        ) : items.map((item: any) => (
          <div key={item.id} className="bg-white border border-gray-100 rounded-sm p-5 flex items-center justify-between hover:shadow-sm transition-shadow">
            <div className="flex items-center gap-4">
              <div className="w-16 h-12 bg-gray-50 rounded-sm flex items-center justify-center overflow-hidden border border-gray-100">
                {item.logo_url ? <img src={item.logo_url} alt={item.name} className="w-full h-full object-contain p-1" /> : <span className="text-xs text-gray-400">No Logo</span>}
              </div>
              <div>
                <div className="font-semibold text-gray-900">{item.name}</div>
                <div className="text-xs text-gray-400 mt-0.5">Order: {item.sort_order}</div>
                <button onClick={() => toggleActive(item)} className="text-xs px-2 py-0.5 rounded-sm font-semibold mt-1" style={{ background: item.is_active ? '#d1fae5' : '#fee2e2', color: item.is_active ? '#065f46' : '#991b1b' }}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-sm w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-lg">{editItem ? 'Edit Partner' : 'Add Partner'}</h3>
              <button onClick={() => setShowModal(false)}><X size={20} /></button>
            </div>
            <div className="p-6 flex flex-col gap-4">
              <div>
                <label className="text-xs font-semibold uppercase text-gray-500 block mb-1.5">Name *</label>
                <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-teal-700 rounded-sm" placeholder="Partner name" />
              </div>
              <div>
