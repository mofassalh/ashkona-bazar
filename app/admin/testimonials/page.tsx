...
export default function AdminTestimonials() {
  ...
  const [form, setForm] = useState({ text: '', author: '', role: '', sort_order: 0, is_active: true })
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ text: '', author: '', role: '', sort_order: 0, is_active: true })
    setShowModal(true)
  }

  const openEdit = (item: any) => {
    setEditItem(item)
    setForm({ text: item.text || '', author: item.author || '', role: item.role || '', sort_order: item.sort_order || 0, is_active: item.is_active !== false })
    setShowModal(true)
  }

  const handleSave = async () => {
    if (!form.text || !form.author) { toast.error('Text and author are required'); return }
    const data = { text: form.text, author: form.author, role: form.role, sort_order: parseInt(form.sort_order) || 0, is_active: form.is_active }
    if (editItem) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          ...
          <div className="col-span-2 text-center py-10 text-gray-400">No testimonials yet</div>
        ) : items.map((item: any) => (
          <div key={item.id} className="bg-white border border-gray-100 rounded-sm p-5 hover:shadow-sm transition-shadow">
            ...
            <p className="text-sm text-gray-600 mb-4 line-clamp-3 italic">{item.text}</p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-sm w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-lg">{editItem ? 'Edit Testimonial' : 'Add Testimonial'}</h3>
              <button onClick={() => setShowModal(false)}><X size={20} /></button>
            </div>
            <div className="p-6 flex flex-col gap-4">
              <div>
                <label className="text-xs font-semibold uppercase text-gray-500 block mb-1.5">Review Text *</label>
                <textarea value={form.text} onChange={e => setForm({...form, text: e.target.value})} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-teal-700 rounded-sm" rows={4} placeholder="Customer review..." />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase text-gray-500 block mb-1.5">Author *</label>
                <input value={form.author} onChange={e => setForm({...form, author: e.target.value})} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-teal-700 rounded-sm" placeholder="Customer name" />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase text-gray-500 block mb-1.5">Role / Title</label>
                <input value={form.role} onChange={e => setForm({...form, role: e.target.value})} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-teal-700 rounded-sm" placeholder="Verified Buyer" />
              </div>
              <div>
