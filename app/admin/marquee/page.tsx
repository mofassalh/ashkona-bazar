...
export default function AdminMarquee() {
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ text: '', sort_order: 0, is_active: true })
    setShowModal(true)
  }

  const openEdit = (item: any) => {
    setEditItem(item)
    setForm({ text: item.text, sort_order: item.sort_order || 0, is_active: item.is_active !== false })
    setShowModal(true)
  }

  const handleSave = async () => {
    if (!form.text) { toast.error('Text is required'); return }
    const data = { text: form.text, sort_order: parseInt(form.sort_order) || 0, is_active: form.is_active }
    if (editItem) {
      const { error } = await supabase.from('marquee_items').update(data).eq('id', editItem.id)
      if (error) { toast.error('Error updating'); return }
      toast.success('Updated!')
    } else {
      const { error } = await supabase.from('marquee_items').insert(data)
    ...
    <div>
      ...
      <div className="bg-white border border-gray-100 rounded-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-400">No marquee items yet</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase text-gray-400">Text</th>
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase text-gray-400">Order</th>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4 text-sm text-gray-800">{item.text}</td>
                  <td className="px-5 py-4 text-sm text-gray-500">{item.sort_order}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-sm w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-lg">{editItem ? 'Edit Marquee Text' : 'Add Marquee Text'}</h3>
              <button onClick={() => setShowModal(false)}><X size={20} /></button>
            </div>
            <div className="p-6 flex flex-col gap-4">
              <div>
                <label className="text-xs font-semibold uppercase text-gray-500 block mb-1.5">Text *</label>
                <input value={form.text} onChange={e => setForm({...form, text: e.target.value})} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-teal-700 rounded-sm" placeholder="Marquee text..." />
              </div>
              <div>
